...
Each well has standing-wave states. The $n=1$ mode has lowest energy as it's like particle in a box (integer $n>0$):
\[
E_n = \frac{n^2 h^2}{8mL^2}.
\]
So state A and state B are shown as standing waves in their respective wells. To convert A $\to$ B, the molecule must pass over the barrier in the middle (the transition state) which will require some activation energy.

\textbf{(b)}
\[
\text{rate}=\frac{k_B T}{h}e^{-E/(k_B T)},\qquad \text{time}=\frac{1}{\text{rate}}.
\]
At $T=310\,\text{K}$,
\[
...
\text{time}&\approx6.5\times10^{-12}\,\text{s}.\\[4pt]
E&=1\,\text{eV}: &
\text{rate}&=(6.46\times10^{12})e^{-1/0.02671}=3.42\times10^{-4}\,\text{s}^{-1}, &
\text{time}&\approx2.9\times10^{3}\,\text{s}.\\[4pt]
E&=10\,\text{eV}: &
